...
def awq_quantize_weight(weight: torch.Tensor, activation_sample: torch.Tensor, bits=8):
    ...
    for oc in range(out_channels):
        ...
        scale = w_absmax / qmax if w_absmax > 1e-8 else 1.0

        w_int = torch.clamp(torch.round(w / scale), -qmax - 1, qmax).int()
        w_dequant = w_int * scale

        new_weight[oc] = w_dequant
...
class ActivationRecorder:
    ...
    def hook(self, module, input, output):
        self.storage.append(input[0].detach().cpu())
